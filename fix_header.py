# Code made by Justin Lim cloned from https://github.com/Corallian-Coral-yan/corallian-cpc-fix/blob/master/fix-cpc.py
...
def main():
    ...
    with open("input_file.txt", "r", encoding="utf-8") as file:
        ...
        for row in reader:
            ...
            print(f"Processing folder: {Fore.GREEN}<{folder_name}>{Style.RESET_ALL} with code file: {Fore.GREEN}<{codefile_path}>{Style.RESET_ALL}")
